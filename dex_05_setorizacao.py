...
def simulando (df_base_clientes,vetoed_grid_df,i,subcoords ):
    ...
    #  print(f'Testando com FTE = {fte_suporte +0.41}...')
        ...
        if com_tempo_limite_de_processamente == 1:
            with st.spinner(f'Aguarde até {tempo_limite} segundos (otimizando)...'):
    ...
    if status_ == 1:
        ...
        for j in range(n):
            if y[j].value() ==1:
                ...
                info_otim.append(f'--> FTE Total: {round(sum(x[(i, j)].value() * Visita[i] * (0.25 + (c[i][j] / 65)) for i in range(m)) / 220 + sum(x[(i, j)].value() * FTE_Atendimento[i] for i in range(m)) + 0.41 ,2)}')

                info_otim.append(f'--> # de PDV:  {round(sum(x[(i, j)].value() * 1 for i in range(m) ) ,0 )}' )
                ...
                info_otim.append(f'--> ROL Total: {round(sum(x[(i, j)].value() * ROL[i] for i in range(m)), 0)}')
    ...
    return  points_to_be_served_2,info_otim
...
with tab1:
    ...
    if base_de_clientes:
        ...
        if criar_visao_grid:
            ...
            list_grid_ , list_veto = visoes_grid_(df_base_clientes,subcoords_)
            ...
            if simular:
                ...
                for i in range(len(subcoords_)):
                    ...
                    df_cluster, info_otim = simulando (df_base_clientes,list_veto[i],i,subcoords)
                    ...
                    fig1 = px.scatter_mapbox(df_cluster, lat="lat", lon="long", title='Visão Final', height=400,width = 1250, color='cluster', hover_data=["Nome rede_ Ajustado", "Nome PDV_Ajustado"])
        ...
        with tab2:
            ...
            if subcoord_mapa and criar_visao_grid:
                ...
                fig3 = px.scatter_mapbox(grid_df, lat="lat", lon="long", title='Visão Grid:', height=500,width = 600)
                fig2 = px.scatter_mapbox(vetoed_grid_df, lat="lat", lon="long", title='Visão de Grid com Pontos de Veto:', height=500, width = 600)
